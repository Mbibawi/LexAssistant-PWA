...
export class Cases extends Folders {
    ...
    async writeNotes(folderName, notes) {
        await this.writeJson(this.notesPath(folderName), { notes });
    }
    ...
    async writeConversation(folderName, messages) {
        await this.writeJson(this.convPath(folderName), { messages });
    }
    ...
    renderCaseList() {
        ...
        for (const c of this.cases) {
            ...
            item.append(el('span', { className: 'case-item__name', textContent: c.name }), el('span', { className: 'case-item__domain', textContent: c.domain }), el('span', { className: `case-item__status case-item__status--${c.status}`, textContent: labels[c.status] }));
    ...
    renderNoteBar() {
        ...
        bar.append(el('span', { className: 'note-badge', textContent: String(n) }), el('span', { className: 'note-bar__text',
            textContent: `note${n > 1 ? 's' : ''} active${n > 1 ? 's' : ''} · ` +
                this.caseNotes.map(x => x.content.slice(0, 40) + '…').join(' — ') }), (() => { const b = el('button', { className: 'note-bar__manage', textContent: 'Gérer' }); b.onclick = () => this.openNotesModal(); return b; })());
    }
    // ─── Render: chat ─────────────────────────────────────────────────────────────
    renderChat() {
        ...
        if (!this.caseMessages.length) {
            ...
            if (!c)
                return;
            area.appendChild(el('div', { className: 'msg msg--assistant' }, el('div', { className: 'msg__label', textContent: 'Lex Assistant' }), el('div', { className: 'msg__bubble' }, el('p', { innerHTML: `Dossier <strong>${c.name}</strong>. ${c.documents.length} pièce(s), ${this.caseNotes.length} note(s)${this.skills.length ? `, ${this.skills.length} skill(s)` : ''}.` }), el('p', { textContent: 'Que souhaitez-vous faire ?' }))));
            return;
        }
    ...
    openCaseFormModal(existing) {
      ...
      <div class="modal-btns">
        ...
        qs('#f-save', dialog).onclick = async () => {
            ...
            const now = Date.now();
            const meta = { name, folderName: folder, domain, status,
    ...
    setupTopbar() {
        ...
        if (odBtn)
            odBtn.onclick = async () => { if (!this.oneDriveUser)
                await this.connectOneDrive();
            else
                await this.refreshCaseFromOneDrive(); };
    }
    setupSidebar() {
        const newSideBtn = document.getElementById('btn-new-case-sidebar');
    ...
    showNotConnected() {
        ...
        if (!area)
            return;
        area.innerHTML = '';
        area.appendChild(el('div', { className: 'empty-state' }, el('div', { className: 'empty-icon', textContent: '☁' }), el('h2', { textContent: 'OneDrive non connecté' }), el('p', { textContent: 'Configurez votre App Registration Azure et connectez-vous.' }), (() => { const b = el('button', { className: 'btn btn--primary', textContent: '☁ Configurer OneDrive' }); b.onclick = () => this.openSettingsModal(); return b; })()));
    }
    sanitiseFolder(name) {
...
export class Library extends Cases {
    ...
    libConvPath(domain) { return `${this.libDomainFolder(domain)}/_conversation.json`; }
    // ─── CRUD ─────────────────────────────────────────────────────────────────────
