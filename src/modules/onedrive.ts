...
class Configuration {
  ...
  root(): string { return this.config?.rootFolder ?? APP_ROOT; }
}
...
export class OneDriveAuth {
  ...
  private readonly CLIENT_ID: string = '9cb553c1-8473-4b2a-91d4-fef8b7cd7bff';
  private readonly TENANT_ID: string = 'f45eef0e-ec91-44ae-b371-b160b4bbaa0c';
  private _scopes: string[] = ['Files.ReadWrite', 'User.Read', 'openid', 'profile'];
  private _MSAL = new MSAL();
  private _msal: MsalApp | null = null;
  ...
  private getMsal(): Promise<MsalApp> {
    ...
    function old() {
      ...
      loading = new Promise((res, rej) => {
        const init = async () => {
          if (!config) throw new Error('OneDrive non configuré');
        ...
        if (msal) { init().then(res).catch(rej); return; }
   ...
   */
  async signIn(msal: MsalApp | null): Promise<string | null> {
    ...
    async function old() {
      if (!msal) msal = await getMsal();
      if (!msal) throw new Error('MSAL instance non disponible');
      ...
      if (!account) throw new Error('MSAL account not found despite redirection');
  ...
  async isSignedIn(): Promise<string | null> {
    try {
      const msal = await this.getMsal();
      const accounts = msal.getAllAccounts();
      if (accounts.length) { this._account = accounts[0]; return this.getSignedInUser(this._account); }
      return null;
    } catch { return null; }
  }
   ...
   */
  getSignedInUser(account: MsalAccount | null = this._account): string | null {
   ...
   */
  async oneDriveProxy(root: string, method: string, payload: { path: string, body?: Uint8Array<ArrayBuffer> | ArrayBuffer, mimeType?: string }) {
    if (!this._userOid) await this.getAccessToken();
    ...
    if (root === 'fetch' && response.ok) return await response.blob();
  ...
  async gFetch(
    ...
    if (path !== ClaudeAPI.PROXY && !this._token) await this.getAccessToken();
...
class GraphAPI {
  ...
  constructor(accessToken: string = '', filePath?: string, sessionId?: string) {
 ...
 * @returns 
 */
  async createAndUploadDocumentFromTemplate(
    templatePath: string,
    savePath: string = this.filePath,
    lang: string,
    tables?: [string, string[][], number][],
    contentControls?: { title?: string, nestedCtrls: [string, string][][] | [string, string][] },
    totalsLabels?: string[]
  ) {
    ...
    function editXML([doc, fileName]: [XMLDocument, string]) {
      ...
      if (contentControls?.title)
        ...
        editContentControls(xml, contentControls?.nestedCtrls as [string, string][], doc);
    };


    function editTables(xml: XML, doc: XMLDocument) {
      if (!tables) return;
    ...
    function editContentControls(xml: XML, contentControls: [string, string][] | undefined, parent: XMLDocument | Element) {
    ...
    function editRepeatingControls(xml: XML, options: { title?: string, nestedCtrls: [string, string][][] | [string, string][] } | undefined, parent: XMLDocument) {
      if (!options || !options.title || !options.nestedCtrls) return;

      const title = options.title;
      // We must assert the type here because options.nestedCtrls could be either the flat or nested array
      const nestedCtrls = options.nestedCtrls as [string, string][][];

      const allCtrls = xml.getContentControls(parent);

      // Find the master layout we want to duplicate
      const masterNodes = xml.findContentControlsByTitle(allCtrls, title) as Element[];
      if (!masterNodes?.length) return;
      const master = masterNodes[0];

      // Process each array of data
      nestedCtrls.forEach((controlData: [string, string][]) => {
        // You must use cloneNode(true) for a deep clone of the content block
        const clone = master.cloneNode(true) as Element;
  ...
  private async getXmlDocs(templatePath: string) {
...
class Folders extends OneDriveAuth {
   ...
   * @param folderPath The path to the folder.
   */
  protected async listAllFolderItems(folderPath: string): Promise<GraphDriveItem[]> {
...
abstract class Common extends Folders {
  ...
  protected abstract renderChat(): void;
  protected abstract buildMsgEl(msg: LibConversationMessage): HTMLElement;
  protected abstract sendMessage(ta: HTMLTextAreaElement, sendBtn: HTMLButtonElement): Promise<void>;
  protected abstract updateQuickPrompts(ta: HTMLTextAreaElement): void;
   ...
   * @returns 
   */
  protected setupInputArea(): void {
    const ta = byID(ids.userInput) as HTMLTextAreaElement | null;
    const sendBtn = byID(ids.sendBtn) as HTMLButtonElement | null;
    if (!ta || !sendBtn) return;
    ta.addEventListener('input', () => this.autoResize(ta));
    ta.addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); this.sendMessage(ta, sendBtn); } });
  ...
  openSettingsModal(): void {
    ...
    qs<HTMLButtonElement>('#s-od-init', dialog).onclick = async () => {
      if (!oneDrive.isConfigured) { toast('Sauvegardez la configuration d\'abord.', 'error'); return; } //!might need to be change to if(!oneDrive.userOid)
      try {
...
export class Cases extends Common {
  ...
  private _foldersMeta: FolderMeta[] = [];
  private _activeCase: FolderMeta | null = null;
  private _caseNotes: PermanentNote[] = [];
  private _caseMessages: ChatMessage[] = [];
  private _caseKb: string | null = null; // cached knowledge base content

  // ─── Path helpers ─────────────────────────────────────────────────────────

  private casePath = (f: string) => `${this.root}/${this.mainFolder}/${f}`;
  ...
  private readCaseMeta(f: string) { return this.readJson<CaseMeta>(this.metaPath(f)); }
  private async writeCaseMeta(f: string, m: CaseMeta) {
  ...
  private async readConversation(f: string) { return (await this.readJson<ConversationFile>(this.convPath(f)))?.messages ?? []; }
  private writeConversation(f: string, m: ChatMessage[]) { return this.writeJson(this.convPath(f), { messages: m } satisfies ConversationFile); }
  ...
  private async saveMeta(): Promise<void> {
    ...
    try { await this.writeCaseMeta(this._activeCase.folderName, meta); }
    finally { this._saving = false; }
  }

  private saveNotes() { return this._activeCase ? this.writeNotes(this._activeCase.folderName, this._caseNotes) : Promise.resolve(); }
  private saveConversation() { return this._activeCase ? this.writeConversation(this._activeCase.folderName, this._caseMessages) : Promise.resolve(); }
  ...
  protected async loadAllSubFolders(): Promise<void> {
    ...
    this.renderCaseList();
    if (this._foldersMeta.length > 0) await this.selectCase(this._foldersMeta[0].folderName);
  ...
  private async selectCase(folderName: string): Promise<void> {
    ...
    const kb = await this.claude.loadLatestCaseKb(
      this.casePath(folderName),
      this.listAllFolderItems,
      this.readFilePath,
    );
    if (kb) this._caseKb = kb.content;
  ...
  private renderCaseList(): void {
    ...
    for (const c of this._foldersMeta) {
      ...
      item.append(
        el('span', { className: 'case-item_name', textContent: c.name }),
        el('span', { className: 'case-item_domain', textContent: c.domain }),
        el('span', { className: `case-item_status case-item_status--${c.status}`, textContent: labels[c.status] }),
      );
  ...
  protected renderDocList(): void {
    ...
    for (const doc of [...docs].sort((a, b) => b.addedAt - a.addedAt)) {
      ...
      item.append(el('span', { className: 'doc-icon', textContent: mimeIcon(doc.mimeType) }), info,
        el('span', { className: `doc-kind doc-kind--${doc.kind}`, textContent: kindLabel(doc.kind) }), del);
      list.appendChild(item);
    }
  }
  ...
  private renderNoteBar(): void {
    if (!this._caseNotes.length) return;
  ...
  protected renderChat(): void {
    ...
    for (const msg of this._caseMessages) area.appendChild(this.buildMsgEl(msg));
    (area as HTMLElement).scrollTop = (area as HTMLElement).scrollHeight;
  }

  protected buildMsgEl(msg: ChatMessage): HTMLElement {
    const wrap = el('div', { className: `msg msg--${msg.role}` });
    const bubble = el('div', { className: 'msg_bubble' });
    bubble.innerHTML = renderMarkdown(msg.content as string);
    ...
    if (msg.role === 'assistant') {
      ...
      copy.onclick = () => { navigator.clipboard.writeText(msg.content as string); toast('Copié.', 'info', 1500); };
      ...
      if (msg.mode === 'redaction' || msg.generatedDocName) {
        ...
        dl.onclick = async () => {
          try {
            const blob = await generateDocx({ title: msg.generatedDocName ?? 'Document', content: msg.content as string, caseRef: this._activeCase?.name ?? '' });
  ...
  protected async sendMessage(ta: HTMLTextAreaElement, sendBtn: HTMLButtonElement): Promise<void> {
    if (!ta || !this._activeCase) return;
  ...
  private setupBarsBtns(): void {
    ...
    this.onClick(btn(ids.btnCaseSummary), async () => {
      ...
      await this.sendMessage(byID(ids.toast) as HTMLTextAreaElement, byID(ids.sendBtn) as HTMLButtonElement);
    });

    qsa<HTMLButtonElement>('.doc-filter-tab').forEach((tab) =>
      this.onClick(tab, () => {
  ...
  private async clearConversation(folderName: string): Promise<void> {
    ...
    if (this._activeCase?.folderName === folderName) this.renderChat();
...
export class Library extends Common {
  ...
  private _libKb: string | null = null; // cached knowledge base for active domain
  ...
  private libDomainPath(domain: string): string {
    return `${this.root}/${this.mainFolder}/${this.domainLabel(domain)}`;
  }
  private libMetaPath(domain: LibDomain): string {
    return `${this.libDomainPath(domain)}/_meta.json`;
  }
  private getConvPath(): string {
  ...
  async readLibMeta(domain: LibDomain): Promise<LibDomainMeta | null> {
  ...
  private async readLibConversation(): Promise<LibConversationMessage[]> {
    return (await this.readJson<{ messages: LibConversationMessage[] }>(this.getConvPath()))?.messages ?? [];
  }
  private writeLibConversation(messages: LibConversationMessage[]): Promise<void> {
  ...
  async buildKnowledgeBase(appendMode: Boolean = false): Promise<void> {
    if (this.activeDomain === 'all') { toast('Sélectionnez un domaine spécifique pour générer une base de connaissance.', 'info'); return; }
    const btn = byID(ids.btnBuildKb) as HTMLButtonElement | null;
    if (btn) { btn.disabled = true; btn.textContent = '🧠 Génération…'; }
    try {
      ...
      const markdown = await this.claude.buildLibKnowledgeBase(
        this.activeDomain,
        docs,
        (name) => this.readLibFile(this.activeDomain as LibDomain, name),
        [],
        appendMode,
      );
  ...
  async switchDomain(domain: LibDomain | 'all'): Promise<void> {
    ...
    if (domain !== 'all') {
      ...
      const kb = await this.claude.loadLatestCaseKb(
        this.libDomainPath(domain),
        (p) => this.listAllFolderItems(p),
        (p) => this.readFilePath(p),
      );
      if (kb) this._libKb = kb.content;
    }
    this.libMessages = await this.readLibConversation().catch(() => []);
    this.renderDomainPills();
    this.renderDocList();
    ...
    this.updateQuickPrompts(byID(ids.toast) as HTMLTextAreaElement);
  }
  ...
  protected renderDocList(): void {
    ...
    if (this.activeDomain === 'all') { for (const [, d] of this.domainDocs) docs.push(...d); }
    else { docs = this.domainDocs.get(this.activeDomain) ?? []; }
    ...
    for (const doc of docs) {
      ...
      del.onclick = async (e) => {
        e.stopPropagation();
        if (!await confirm(`Retirer "${doc.name}" de la bibliothèque ? (Fichier OneDrive conservé.)`)) return;
  ...
  protected renderChat(): void {
    ...
    for (const msg of this.libMessages) area.appendChild(this.buildMsgEl(msg));
    (area as HTMLElement).scrollTop = (area as HTMLElement).scrollHeight;
  }

  protected buildMsgEl(msg: LibConversationMessage): HTMLElement {
    const wrap = el('div', { className: `msg msg--${msg.role}` });
    const bubble = el('div', { className: 'msg_bubble' });
  ...
  protected async sendMessage(ta: HTMLTextAreaElement, sendBtn: HTMLButtonElement): Promise<void> {
    ...
    if (this.activeDomain === 'all') {
      ...
      for (const d of this.DOMAINS) {
        if (!this.domainDocs.has(d.id)) docs.push(...(await this.loadDomainMeta(d.id)));
      }
    } else {
