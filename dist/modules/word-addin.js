...
export class DocumentContext {
    ...
    async getSelection() {
        if (!this.isReady)
            throw new Error('DocumentContext.init() not called');
        if (this.host === hosts.word)
            return this._getWordSelection();
        if (this.host === hosts.excel)
            return this._getExcelSelection();
        return null;
    }
    _getWordSelection() {
        return new Promise((resolve) => {
            ...
            Office.context.document.getSelectedDataAsync(Office.CoercionType.Text, (textResult) => {
                ...
                Office.context.document.getSelectedDataAsync(Office.CoercionType.Ooxml, (xmlResult) => {
                    resolve({
                        text,
                        ooxml: xmlResult.status === Office.AsyncResultStatus.Succeeded
                            ? xmlResult.value
                            : undefined,
                    });
                });
     ...
     */
    async getSnapshot(includeFullBody = false) {
        const base = {
            host: this.host,
    ...
    async _writeWordTrackedChange(text, tracked, replace) {
        await Word.run(async (context) => {
            ...
            selection.insertText(text, replace ? Word.InsertLocation.replace : Word.InsertLocation.after);
            await context.sync();
            if (tracked) {
    ...
    async _writeExcelSelection(text, replace) {
     ...
     */
    async saveSharedContext(history = []) {
        const snapshot = await this.getSnapshot(false);
        let existing = null;
        try {
            existing = await this.readJson(this.CONTEXT_FILE);
        }
        catch {
        ...
        const updated = {
            ...
            excelSnapshot: this.host === hosts.excel ? snapshot : existing?.excelSnapshot,
     ...
     */
    async loadSharedContext(graphFetch) {
        try {
            const res = await graphFetch(`/me/drive/root:/${this.CONTEXT_FILE}:/content`);
            if (!res.ok)
                return null;
    ...
    _getWordHeadings() {
        return Word.run(async (ctx) => {
            ...
            return headings.items
                .filter((p) => ['Heading 1', 'Heading 2', 'Heading 3'].includes(p.style))
                .map((p) => {
                const indent = p.style === 'Heading 1'
                    ? ''
                    : p.style === 'Heading 2'
                        ? '  '
                        : '    ';
...
//# sourceMappingURL=word-addin.js.map