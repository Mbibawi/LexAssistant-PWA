...
// ─── Handlers ─────────────────────────────────────────────────────────────────
async function handleClaudeProxy(req, res, azure = false) {
    const claudePath = req.body?.path;
    if (!claudePath) {
        res.status(400).json({ error: "Missing body parameter: path" });
        return;
    }
    if (!req.body.path || !req.body.messages)
        throw new Error("Missing body parameter: path or messages");
    try {
...
async function callAzureFoundry(req) {
    //Azure AI Foundry
    if (!FOUNDRY_API_KEY)
...
async function fetchAPI(url, apiKey, req) {
...
//# sourceMappingURL=claudeAi-proxy.js.map